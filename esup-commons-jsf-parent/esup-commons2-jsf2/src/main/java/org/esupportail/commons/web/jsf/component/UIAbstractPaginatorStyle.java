...
import org.esupportail.commons.jsf.ResourceBundlePhaseListener;
...
public abstract class UIAbstractPaginatorStyle extends UIComponentBase {
	...
	protected Div makeDivNbItems(final Application a) {
		...
		divNbItems.setId(this.getId() + "_nb_items");
		...
		if (getBundles() != null) {
			...
			span.setValue(getBundles().get(ITEMS_NUMBERS_I18N_KEY));
			span.setParent(divNbItems);
			span.setId(divNbItems.getId() + "_numbers_items");
			...
			if (UIComponentTag.isValueReference(getItemsName())) {
				param.setValueExpression(JSFAttr.VALUE_ATTR, getExpression(getItemsName(), String.class, a));
			} else {
	...
	protected Div makeDivNbItemsByPage(final Application a) {
		...
		divNbItemsByPage.setId(this.getId() + "_nb_items_by_page");
		divNbItemsByPage.setStyleClass(ITEMS_PER_PAGE_BLOCK);
		if (getBundles() != null) {
			HtmlOutputFormat span = (HtmlOutputFormat) a.createComponent(HtmlOutputFormat.COMPONENT_TYPE);
			span.setValue(getBundles().get(ITEMS_PER_PAGE_I18N_KEY));
			span.setParent(divNbItemsByPage);
			span.setId(divNbItemsByPage.getId() + "_items_per_page");
			span.setStyleClass(TagsConfigurator.getInstance().getTextStyleClass());
			...
			selectMenu.setId(divNbItemsByPage.getId() + "_select_pagesize");
			selectMenu.setValueExpression(JSFAttr.VALUE_ATTR, getExpression(
					TagUtils.makeELExpression(getPaginatorELExpression() 
							+ "." + PaginatorUtils.PAGE_SIZE), String.class, a));
			selectMenu.setParent(divNbItemsByPage);
			if (StringUtils.hasText(getOnchange())) {
				selectMenu.setOnchange(getOnchange());
			}
			
			for (String item : getPaginator().getPageSizeItems()) {
	 ...
	 */
	public ValueExpression getExpression(String expression, Class< ? > result, Application a) {
		if (result.equals(String.class)) {
			return TagUtils.getStringValueExpression(a, expression);
		}
		return a.getExpressionFactory()
			.createValueExpression(
					getFacesContext().getELContext(), expression, result);
	}
	...
	public Paginator< ? > getPaginator() {
		if (paginator == null) {
			TagValueExpression v = (TagValueExpression) this.getValueExpression(UIPaginator.PAGINATOR_ATTR);
			paginator = (Paginator< ? >) v.getValue(getFacesContext().getELContext()); 
		}
		return paginator;
	}
	...
	protected String getStringsVar() {
		if (stringsVar == null) {
			stringsVar = ResourceBundlePhaseListener.DEFAULT_STRINGVAR;
		}
		return stringsVar;
	}
